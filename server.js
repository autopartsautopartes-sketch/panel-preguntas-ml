...
function makeEngine(deps) {
  ...
  async function fetchSales(account, from, to) {
    ...
    if (from) p['order.date_created.from'] = from + 'T00:00:00.000-00:00';
    if (to) p['order.date_created.to'] = to + 'T23:59:59.000-00:00';
    const orders = [];
  ...
  return { getAdvertiser, getAdvertiserId, listCampaigns, listCampaignItems, setCampaignStatus, fetchItems, fetchSales, fetchShipping };
}
...
// Segmenta un producto según margen (%) × si vende (velocidad).
//  estrella  = buen margen + vende  → escalar ADS
//  promesa   = buen margen + no vende → ADS + promo para posicionar
//  vaca      = margen flaco + vende  → optimizar precio / sacar promo
//  durmiente = margen flaco + no vende → revisar precio o activar del stock
function segmentItem(marginPct, sales, cfg) {
  const high = (cfg && cfg.marginHigh != null) ? cfg.marginHigh : 12;
  const minSales = (cfg && cfg.minSales != null) ? cfg.minSales : 1;
  const buen = (marginPct != null) && marginPct >= high;
  const vende = (sales || 0) >= minSales;
  if (buen && vende) return 'estrella';
  if (buen && !vende) return 'promesa';
  if (!buen && vende) return 'vaca';
  return 'durmiente';
}
function segmentAction(seg, isLeader, liderName, stock) {
  if (isLeader === false) return 'Solo orgánico — no anunciar (líder: ' + (liderName || '—') + ')';
  if (stock && (seg === 'vaca' || seg === 'durmiente')) return 'Stock propio: candidato ideal para PROMO (rotás tu stock)';
  const m = {
    estrella: 'Escalar ADS · ROAS 5x · sin promo (no regales margen)',
    promesa: 'ADS + promo para posicionar · ROAS 7-8x (inversión de ranking)',
    vaca: 'Optimizar precio / sacar promo innecesaria (recuperás margen)',
    durmiente: 'Revisar precio o activar del stock disponible',
  };
  return m[seg] || '—';
}
...
function buildStrategy(costsTable, hist, accountsMeta, cfg) {
  ...
  for (const id in costsTable) {
    ...
    const realMargin = agg && agg.mN > 0 ? agg.mSum / agg.mN : null;
    const listMargin = (Number(c.marginList) || 0) * 100;
    const marginPct = realMargin != null ? realMargin : (listMargin || null);
    const salesN = agg ? agg.sales : 0;
    const seg = segmentItem(marginPct, salesN, cfg);
    items.push({
      ...
      margin: marginPct, marginReal: realMargin != null, sales: salesN, revenue: agg ? agg.rev : 0, net: agg ? agg.net : 0,
      stock: !!c.stock, segment: seg,
    });
  }
  ...
  for (const it of items) {
    ...
    it.action = segmentAction(it.segment, it.isLeader, it.leaderName, it.stock);
  }

  // 5) Resúmenes: por segmento (global) y por cuenta.
  const segCount = { estrella: 0, promesa: 0, vaca: 0, durmiente: 0 };
  const perAcc = {};
  const spanDays = (ledgerMinDate && ledgerMaxDate) ? Math.max(1, daysBetween(ledgerMinDate, ledgerMaxDate)) : 0;
  for (const it of items) {
    segCount[it.segment]++;
    const a = perAcc[it.account_id] || { account_id: it.account_id, account_name: it.account_name, objetivo: it.objetivo, items: 0, estrella: 0, promesa: 0, vaca: 0, durmiente: 0, lider: 0, orgOnly: 0, dup: 0, rev: 0, net: 0 };
    a.items++; a[it.segment]++; a.rev += it.revenue; a.net += it.net;
...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/estrategia', async (req, res) => {
    if (!requireAuth(req)) return sendJSON(res, 401, { error: 'No autorizado' });
    const u = new URL(req.url, 'http://localhost');
    const marginHigh = Number(u.searchParams.get('marginHigh'));
    const minSales = Number(u.searchParams.get('minSales'));
    ...
    const cfg = { marginHigh: isNaN(marginHigh) ? 12 : marginHigh, minSales: isNaN(minSales) ? 1 : minSales, tieBand: 2 };
    try {
      const strat = buildStrategy(costsTable, hist, accountsMeta, cfg);
      // Para la tabla/descarga mandamos un tope de items (priorizando duplicados y estrellas).
      const rank = { estrella: 0, promesa: 1, vaca: 2, durmiente: 3 };
      const items = strat.items.slice().sort((a, b) =>
        (b.duplicated - a.duplicated) || (rank[a.segment] - rank[b.segment]) || ((b.margin || -999) - (a.margin || -999))
      ).slice(0, 500);
