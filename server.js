...
function makeEngine(deps) {
  ...
  return { getAdvertiser, getAdvertiserId, listCampaigns, listCampaignItems, setCampaignStatus, fetchItems, fetchSales, fetchShipping, simulateFee };
}
...
function buildStrategy(costsTable, hist, accountsMeta, cfg) {
  ...
  for (const id in costsTable) {
    ...
    // MARGEN de estrategia: SIEMPRE el del simulador de ML (comisión+cargo fijo+cuotas+envío+impuestos),
    // nunca el margen real de una venta puntual (que varía con la promo). Fallback: margen de lista.
    ...
    // Las ventas del ledger se usan solo para la VELOCIDAD (vende / no vende), no para el margen.
    ...
    items.push({
      ...
      margin: marginPct, marginSrc, sales: salesN, revenue: agg ? agg.rev : 0, net: agg ? agg.net : 0,
...
function registerAds(deps) {
  ...
  // REFINAR margen real por cuenta: usa el simulador de ML para la comisión exacta y guarda
  // simFee/simMargin en la tabla de costos. Tope por velocidad; prioriza los de mayor margen de lista.
  route('GET', '/api/ads/estrategia/refinar', async (req, res) => {
    ...
    const cfg = getAccountCfg(account);
    // Calibración opcional del simulador (para que "Recibís" cuadre con la web de ML).
    ...
    // Ítems de esta cuenta con precio de lista, ordenados por margen de lista desc.
    const ids = Object.keys(table)
      .filter(id => String(table[id].seller_id) === String(account.seller_id) && /^MLA/i.test(id))
      .sort((a, b) => (Number(table[b].marginList) || 0) - (Number(table[a].marginList) || 0))
      .slice(0, cap);
    if (!ids.length) return sendJSON(res, 200, { refined: 0, note: 'Esta cuenta no tiene productos importados.' });
    try {
      // Traigo category_id + precio + tipo en vivo (multiget de a 20).
      const info = await engine.fetchItems(account, ids, 'id,price,category_id,listing_type_id');
      let refined = 0, failed = 0;
      for (const id of ids) {
        const c = table[id]; const it = info[id];
        const price = (it && Number(it.price)) || Number(c.listPrice) || 0;
        const catId = it && it.category_id;
        const lt = (it && it.listing_type_id) || c.listingType;
        ...
        c.simFee = sim.sale_fee; c.simFixed = sim.fixed_fee; c.simCuotas = nm ? nm.cuotas : 0;
        c.simEnvio = nm ? nm.envio : 0; c.simImp = nm ? nm.impuestos : 0; c.simRecibis = nm ? nm.recibis : null;
        c.simMargin = nm ? nm.marginPct : null;
        c.category_id = catId || c.category_id; c.simPrice = price; c.simAt = new Date().toISOString();
        refined++;
      }
      saveCostsFile({ ...file, costs: table, updated: new Date().toISOString() });
      sendJSON(res, 200, { refined, failed, scanned: ids.length, cap, account: account.name, note: ids.length >= cap ? `Refiné los ${cap} de mayor margen de lista (tope por velocidad). Volvé a correr para más.` : 'Refiné todos los productos de la cuenta.' });
    } catch (e) {
      sendJSON(res, 500, { error: (e && e.response && e.response.data) || String(e.message || e) });
    }
  });
