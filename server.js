...
function makeEngine(deps) {
  ...
  // Visitas por publicación (demanda real). Si se pasa [from,to], usa el endpoint por RANGO de fechas
  // (/items/visits?ids=...&date_from=...&date_to=..., máx 150 días) para medir la MISMA ventana que las ventas.
  // Si no, o si el rango falla, cae al acumulado (/visits/items). Lotes EN PARALELO.
  async function fetchVisits(account, ids, from, to) {
    ...
    await mapLimit(batches, 6, async (batch) => {
      try {
        if (useRange) {
          const data = await mlGet('https://api.mercadolibre.com/items/visits', token, { ids: batch.join(','), date_from: from, date_to: to });
          parseInto(data);
        } else {
          parseInto(await mlGet('https://api.mercadolibre.com/visits/items', token, { ids: batch.join(',') }));
        }
      } catch (e) {
        // Fallback al acumulado si el rango no está soportado o falla.
        try { parseInto(await mlGet('https://api.mercadolibre.com/visits/items', token, { ids: batch.join(',') })); } catch (e2) {}
      }
    });
    return out;
  }
...
function registerAds(deps) {
  ...
  function getAccountCfg(account) {
    ...
    return { ...g, objetivo: a.objetivo != null ? a.objetivo : 100000000, taxPct: a.taxPct != null ? a.taxPct : 5, cuotasPct: a.cuotasPct != null ? a.cuotasPct : (g.cuotasPct || 0) };
  }
  ...
  route('GET', '/api/ads/estrategia/refinar', async (req, res) => {
    ...
    try {
      ...
      // fetchItems (multiget) + visitas de 90 días + visitas acumuladas (histórico), TODO en paralelo.
      const [info, visits90, visitsAcc] = await Promise.all([
        ...
        engine.fetchVisits(account, ids),                                       // visitas ACUMULADAS (histórico) como respaldo
      ]);
      ...
      // Precio de VENTA real: el que paga el comprador. Con promo/deal, ML lo expone en sale_price.amount
      // (más bajo que price). Usamos ese; si no hay, el price normal. original_price = precio tachado.
      ...
      for (const id of ids) {
        ...
        const listP = Number(it.price) || Number(c.listPrice) || 0;    // precio de lista (sin deal)
  ...
  route('POST', '/api/ads/costs', async (req, res) => {
    ...
    if (Array.isArray(body.items)) {
      arr = body.items.map(a => ({ item_id: a[0], cost: a[1], costShip: a[2], ship: a[3], commission: a[4], listingType: a[5], listPrice: a[6], marginList: a[7], oferta: a[8], title: a[9], sku: a[10] }));
    } else {
