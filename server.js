...
function makeEngine(deps) {
  ...
  // Envío REAL que absorbió el vendedor en un envío de ML (no la estimación del Excel).
  async function fetchShippingCost(account, shipmentId) {
    if (!shipmentId) return null;
    const token = await getValidToken(account);
    // 1) endpoint de costos del envío (el más preciso para lo que paga el vendedor)
    try {
      const c = await mlGet(`https://api.mercadolibre.com/shipments/${shipmentId}/costs`, token, {}, { 'x-format-new': 'true' });
      if (c) {
        if (Array.isArray(c.senders) && c.senders[0] && c.senders[0].cost != null) return Number(c.senders[0].cost) || 0;
        if (c.gross_amount != null) return Math.max(0, Number(c.gross_amount) - (Number(c.receiver_shipping_cost) || 0));
      }
    } catch (e) {}
    // 2) fallback: costo base del envío
    try {
      const s = await mlGet(`https://api.mercadolibre.com/shipments/${shipmentId}`, token, {}, { 'x-format-new': 'true' });
      if (s && s.base_cost != null) return Number(s.base_cost) || 0;
    } catch (e) {}
    return null; // si no se pudo, el cálculo cae al envío estimado del Excel
  }

  return { getAdvertiser, getAdvertiserId, listCampaigns, listCampaignItems, setCampaignStatus, fetchItems, fetchSales, fetchShippingCost };
}
...
async function analyzeVentas(engine, account, cfg, costs, from, to) {
  ...
  for (const o of orders) {
    let ship = null;
    if (idx < SHIP_CAP) { ship = await engine.fetchShippingCost(account, o.shipment_id); if (ship != null) shipFetched++; }
    idx++;
    const n = o.items.length || 1;
    for (const it of o.items) {
      const shipPart = (ship != null) ? ship / n : null;  // envío real de ML repartido entre ítems de la orden
      const r = saleNet(it, costs[String(it.item_id)], cfg, shipPart);
      facturacion += r.revenue;
      if (r.known) { ganancia += r.net; factConocida += r.revenue; conocidas++; if (r.net < 0) perdida++; } else sinCosto++;
      rows.push({ order_id: o.order_id, date: o.date, item_id: it.item_id, title: it.title, ...r });
    }
  }
  return {
  ...
  route('GET', '/api/ads/ventas/selftest', async (req, res) => {
    ...
    try {
      ...
      if (shipId) {
        ...
        out.shipping_cost_computed = await engine.fetchShippingCost(account, shipId);
