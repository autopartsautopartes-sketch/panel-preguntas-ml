...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/ventas', async (req, res) => {
    ...
    try {
      ...
      if (v.freshCount > 0 || v.taxFixedCount > 0 || v.costFixedCount > 0) { hist.updated = new Date().toISOString(); saveHistFile(hist); }  // persistimos nuevas + correcciones de impuesto + costos actualizados con la lista
  ...
  // + costo del catálogo) es más pesado. El front lo pide aparte y con throttle.
