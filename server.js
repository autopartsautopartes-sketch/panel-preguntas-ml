...
function makeEngine(deps) {
  ...
  async function fetchVisits(account, ids, from, to) {
    ...
    if (useRange) {
      await mapLimit(ids, 20, async (id) => {
        try {
...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/estrategia/refinar', async (req, res) => {
    ...
    if (!account) return sendJSON(res, 404, { error: 'account_id inválido' });
    const u = new URL(req.url, 'http://x');
    const cap = Math.min(Number(u.searchParams.get('cap')) || 80, 600);   // lote chico: las visitas se piden de a una → cada pasada termina rápido (evita que Render corte)
    const cfg = getAccountCfg(account);
    ...
    try {
      ...
      await mapLimit(simJobs, 20, async (job) => { if (job.price > 0) { try { simMap[job.id] = await engine.simulateFee(account, job.price, job.catId, job.lt); } catch (e) {} } });
