...
// Marca si una orden esta "muerta" (cancelada o con devolucion al comprador): no sirve para rentabilidad.
function isDeadOrder(order, shipStatus) {
  const st = String(shipStatus || '').toLowerCase();
  if (['cancelled', 'canceled', 'returned', 'not_delivered'].includes(st)) return true;
  if (String(order.status || '').toLowerCase() === 'cancelled') return true;
  const tags = (order.tags || []).map(t => String(t).toLowerCase());
  if (tags.some(t => ['refunded', 'return', 'returned', 'not_delivered', 'reversed'].includes(t))) return true;
  return false;
}
...
function saleNet(sale, costRow, cfg, realShip) {
  ...
  if (!costRow) { const e = shipReal ? realShip : 0; return { revenue, fee, envio: e, envioReal: shipReal, tax, queda: revenue - fee - e - tax, cost: null, net: null, marginPct: null, known: false }; }
  ...
  return { revenue, fee, envio, envioReal: shipReal, tax, queda, cost, net, marginPct: revenue > 0 ? (net / revenue) * 100 : null, known: true };
}
...
async function analyzeVentas(engine, account, cfg, costs, from, to, hist) {
  ...
  let hidden = 0, savedCount = 0, freshCount = 0;      // ocultas (canceladas/devol.), congeladas, nuevas
  ...
  for (const o of orders) {
    ...
    idx++;
    // Ocultar canceladas y reclamos con devolución al comprador: no sirven para rentabilidad.
    ...
    for (const it of o.items) {
      ...
      if (frozen) {
        ...
        r = { revenue: frozen.revenue, fee: frozen.fee, envio: frozen.envio, envioReal: frozen.envioReal, tax: frozen.tax, queda: frozen.queda, cost: frozen.cost, net: frozen.net, marginPct: frozen.marginPct, known: frozen.known };
        savedCount++;
      } else {
        r = saleNet(it, cr, cfg, shipPart);
        if (sales) {
  ...
  return {
    from, to, days: daysBetween(from, to), count: rows.length, orders: orders.length,
    facturacion, ganancia, margin: factConocida > 0 ? (ganancia / factConocida) * 100 : null,
    conocidas, sinCosto, perdida, shipFetched, hidden, savedCount, freshCount, rows,
  };
}
...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/ventas', async (req, res) => {
    ...
    try {
      ...
      sendJSON(res, 200, {
        ...
        resumen: { count: v.count, orders: v.orders, facturacion: v.facturacion, ganancia: v.ganancia, margin: v.margin, conocidas: v.conocidas, sinCosto: v.sinCosto, perdida: v.perdida, days: v.days, shipFetched: v.shipFetched, hidden: v.hidden, saved: v.savedCount, fresh: v.freshCount },
  ...
  route('POST', '/api/ads/costs', async (req, res) => {
    ...
    // Conteo por cuenta para el estado del panel.
    const byAcc = {};
    for (const id in table) { const k = table[id].account_name || 'sin cuenta'; byAcc[k] = (byAcc[k] || 0) + 1; }
    sendJSON(res, 200, { ok: true, imported: n, total: Object.keys(table).length, account: accName, by_account: byAcc });
  });
  route('GET', '/api/ads/costs', async (req, res) => {
    ...
    const byAcc = {};
    for (const id in table) { const k = table[id].account_name || 'sin cuenta'; byAcc[k] = (byAcc[k] || 0) + 1; }
    sendJSON(res, 200, { total: Object.keys(table).length, updated: f.updated || null, by_account: byAcc });
  });
