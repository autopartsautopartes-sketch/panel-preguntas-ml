...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/estrategia/refinar', async (req, res) => {
    ...
    const u = new URL(req.url, 'http://x');
    const cap = Math.min(Number(u.searchParams.get('cap')) || 15, 600);   // lote moderado; con concurrencia baja (6) + paracaídas + candado, estable
    const cfg = getAccountCfg(account);
