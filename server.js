...
async function analyzeVentas(engine, account, cfg, costs, from, to) {
  ...
  for (const o of orders) {
    ...
    for (const it of o.items) {
      ...
      const r = saleNet(it, cr, cfg, shipPart);
      facturacion += r.revenue;
      if (r.known) { ganancia += r.net; factConocida += r.revenue; conocidas++; if (r.net < 0) perdida++; } else sinCosto++;
      rows.push({ order_id: o.order_id, date: o.date, item_id: it.item_id, title: it.title, status, stock: !!(cr && cr.stock), ...r });
    }
  }
  return {
    ...
    conocidas, sinCosto, perdida, shipFetched, rows,
  };
}
...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/ventas', async (req, res) => {
    ...
    const costs = loadCosts(loadDB);
    try {
      const v = await analyzeVentas(engine, account, cfg, costs, from, to);
      ...
      sendJSON(res, 200, {
        ...
        resumen: { count: v.count, orders: v.orders, facturacion: v.facturacion, ganancia: v.ganancia, margin: v.margin, conocidas: v.conocidas, sinCosto: v.sinCosto, perdida: v.perdida, days: v.days, shipFetched: v.shipFetched },
