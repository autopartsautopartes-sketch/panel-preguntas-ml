...
function makeEngine(deps) {
  ...
  async function fetchSales(account, from, to, maxOrders) {
    ...
    const mapOrder = (o) => {
      ...
      let taxReal = null;
      for (const pm of (o.payments || [])) { const t = Number(pm.taxes_amount); if (!isNaN(t)) taxReal = (taxReal || 0) + t; }
      return {
        ...
        status: o.status, tags: o.tags || [], tax_real: taxReal,
...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/estrategia/refinar', async (req, res) => {
    ...
    if (!account) return sendJSON(res, 404, { error: 'account_id inválido' });
    const u = new URL(req.url, 'http://x');
    const cap = Math.min(Number(u.searchParams.get('cap')) || 250, 600);   // lote más chico = request más rápido (evita timeouts)
    const cfg = getAccountCfg(account);
