...
function saleNet(sale, costRow, cfg) {
  const revenue = (sale.unit_price || 0) * (sale.qty || 0);
  const fee = sale.sale_fee || 0;
  const tax = revenue * ((cfg.taxPct != null ? cfg.taxPct : 5) / 100);
  if (!costRow) return { revenue, fee, tax, cost: null, net: null, marginPct: null, known: false };
  const rm = realMarginAtPrice(costRow, sale.unit_price, cfg.freeShipThreshold);
  const floor = rm ? rm.floorUsed : 0;
  const cost = floor * (sale.qty || 0);
  const net = revenue - fee - tax - cost;
  return { revenue, fee, tax, cost, net, marginPct: revenue > 0 ? (net / revenue) * 100 : null, known: true, freeShip: rm ? rm.freeShip : false };
}
