...
// DEBUG MP BALANCE — muestra la respuesta exacta de MercadoLibre para diagnosticar saldo
route('GET', '/api/mp-balance-debug', async (req, res) => {
  const sess = requireAuth(req);
  if (!sess || sess.role !== 'admin') return sendJSON(res, 403, { error: 'Acceso denegado' });
  ...
  for (const account of (db.ml_accounts || [])) {
    ...
    if (!token) {
      out.push({ account: account.name, seller_id: sid, error: 'Token ML inválido' });
      continue;
    }

    const urls = [
      {
        label: 'ML balance query access_token',
        url: `https://api.mercadolibre.com/users/${sid}/mercadopago_account/balance?access_token=${encodeURIComponent(token)}`,
        headers: {}
      },
      {
        label: 'ML balance bearer',
        url: `https://api.mercadolibre.com/users/${sid}/mercadopago_account/balance`,
        headers: { Authorization: `Bearer ${token}` }
      },
      {
        label: 'MP balance bearer',
    ...
    for (const u of urls) {
      try {
        const r = await fetch(u.url, { headers: u.headers });
        const text = await r.text();
        let body;
        try { body = text ? JSON.parse(text) : {}; } catch(e) { body = text; }
        attempts.push({ label: u.label, status: r.status, ok: r.ok, body });
      } catch(e) {
        attempts.push({ label: u.label, error: e.message || String(e) });
      }
    }

    out.push({ account: account.name, seller_id: sid, attempts });
  }
