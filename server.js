...
async function analyzeVentas(engine, account, cfg, costs, from, to, hist) {
  ...
  const rows = [];
  // ===== VENTAS EN CARRITO: varias órdenes comparten un mismo ENVÍO (shipment). El envío se cobra
  // UNA sola vez → lo asignamos al ítem de MAYOR valor del paquete; el resto del carrito lleva envío $0.
  // Traemos el envío UNA vez por shipment (más rápido y sin duplicar el costo).
  const shipInfo = {};   // shipment_id -> { cost, status, logistic }
  let sIdx = 0;
  for (const o of orders) {
    const sid = o.shipment_id;
    if (!sid || shipInfo[sid] !== undefined) continue;
    if (sIdx < SHIP_CAP) { const sh = await engine.fetchShipping(account, sid); shipInfo[sid] = { cost: sh.cost, status: sh.status, logistic: sh.logistic }; if (sh.cost != null) shipFetched++; sIdx++; }
    else { shipInfo[sid] = { cost: null, status: null, logistic: null }; }
  }
  // Ítem de MAYOR valor por shipment (solo órdenes vivas) = el que se lleva el envío del carrito.
  // shipCount = cuántos ítems vivos comparten el envío (si es >1, es una venta en carrito).
  const shipOwner = {}, shipCount = {};
  {
    const bestBySid = {};
    for (const o of orders) {
      const sid = o.shipment_id; if (!sid) continue;
      const st = shipInfo[sid] ? shipInfo[sid].status : null;
      if (isDeadOrder(o, st)) continue;
      for (const it of o.items) {
        shipCount[sid] = (shipCount[sid] || 0) + 1;
        const value = (it.unit_price || 0) * (it.qty || 0);
        const b = bestBySid[sid];
        if (!b || value > b.value) bestBySid[sid] = { key: o.order_id + ':' + it.item_id, value };
      }
    }
    for (const sid in bestBySid) shipOwner[sid] = bestBySid[sid].key;
  }
  for (const o of orders) {
    const sid = o.shipment_id;
    const info = sid ? shipInfo[sid] : null;
    const ship = info ? info.cost : null, status = info ? info.status : null, logistic = info ? info.logistic : null;
    ...
    for (const it of o.items) {
      ...
      // Envío del CARRITO: solo el ítem de mayor valor del shipment se lleva el envío real; los demás → 0.
      let shipPart;
      if (ship == null) shipPart = null;
      else if (sid && shipOwner[sid] != null) shipPart = (shipOwner[sid] === itemKey) ? ship : 0;
      else shipPart = (n ? ship / n : ship);
      ...
      const pack = (frozen && frozen.pack != null) ? frozen.pack : !!(sid && shipCount[sid] > 1);   // venta en carrito
      let r;
      if (frozen) {
        ...
        if (sales) {
          sales[key] = { account_id: account.id, seller_id: account.seller_id, order_id: o.order_id, pack_id: o.pack_id, date: o.date, item_id: it.item_id, title: it.title, stock: stockFlag, qty, sku, proveedor, cuotas, flex, pack, savedAt: nowIso, ...r };
          freshCount++;
        }
      }
      ...
      rows.push({ order_id: o.order_id, pack_id: (frozen && frozen.pack_id) || o.pack_id || o.order_id, date: o.date, item_id: it.item_id, title: it.title, status, stock: stockFlag, frozen: !!frozen, qty, sku, proveedor, cuotas, flex, pack, net_sin_flex, cost_stock, ...r });
    }
  }
  return {
