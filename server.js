...
function makeEngine(deps) {
  ...
  async function fetchShipping(account, shipmentId) {
    if (!shipmentId) return { cost: null, status: null, logistic: null };
    const token = await getValidToken(account);
    let cost = null, status = null, logistic = null, s = null;
...
function saleNet(sale, costRow, cfg, realShip, realTax, isFlex) {
  ...
  const net = revenue - fee - envio - tax - cost - factura;        // ganancia: el envío SIEMPRE es costo (igual que antes)
...
async function analyzeVentas(engine, account, cfg, costs, from, to, hist) {
  ...
  let costTotal = 0, costStockTotal = 0, gananciaSinFlex = 0, perdidaMonto = 0, cuotasCount = 0;
  const sales = (hist && hist.sales) || null;
  const nowIso = new Date().toISOString();
  const rows = [];
  let idx = 0;
  for (const o of orders) {
    let ship = null, status = null, logistic = null;
    if (idx < SHIP_CAP) { const sh = await engine.fetchShipping(account, o.shipment_id); ship = sh.cost; status = sh.status; logistic = sh.logistic; if (ship != null) shipFetched++; }
    idx++;
    ...
    for (const it of o.items) {
      ...
      const stockFlag = frozen ? !!frozen.stock : !!(cr && cr.stock);
      const qty = it.qty || 0;
      const sku = (frozen && frozen.sku != null) ? frozen.sku : (cr ? (cr.sku || '') : '');
      const proveedor = (frozen && frozen.proveedor != null) ? frozen.proveedor : (cr ? (cr.proveedor || '') : '');
      ...
      if (frozen) {
        ...
        if (sales) {
          sales[key] = { account_id: account.id, seller_id: account.seller_id, order_id: o.order_id, date: o.date, item_id: it.item_id, title: it.title, stock: stockFlag, qty, sku, proveedor, cuotas, flex, savedAt: nowIso, ...r };
          freshCount++;
        }
      }
      ...
      facturaTotal += (r.factura || 0); quedaTotal += (r.queda || 0); if (cuotas > 1) cuotasCount++;
      ...
      rows.push({ order_id: o.order_id, date: o.date, item_id: it.item_id, title: it.title, status, stock: stockFlag, frozen: !!frozen, qty, sku, proveedor, cuotas, flex, net_sin_flex, cost_stock, ...r });
    }
  }
  return {
    ...
    costTotal, costStockTotal, gananciaSinFlex, perdidaMonto, cuotasCount, factConocida, rows,
  };
}
...
function registerAds(deps) {
  ...
  route('GET', '/api/ads/ventas-all', async (req, res) => {
    ...
    try {
      ...
      for (const account of accounts) {
        ...
        gananciaSinFlex += v.gananciaSinFlex; perdidaMonto += v.perdidaMonto; cuotasCount += v.cuotasCount;
      ...
      sendJSON(res, 200, {
        ...
        resumen: { count: allRows.length, orders, facturacion, ganancia, margin: factConocida > 0 ? (ganancia / factConocida) * 100 : null, conocidas, sinCosto, perdida, days, shipFetched, hidden, saved, fresh, taxReal, unidades, feeTotal, envioTotal, taxTotal, facturaTotal, quedaTotal, costTotal, costStockTotal, gananciaSinFlex, perdidaMonto, cuotasCount },
  ...
  async function gestionComputeDay(date, accountId) {
    ...
    const allRows = [];
    const T = { facturacion: 0, unidades: 0, feeTotal: 0, envioTotal: 0, taxTotal: 0, facturaTotal: 0, quedaTotal: 0, costTotal: 0, costStockTotal: 0, ganancia: 0, gananciaSinFlex: 0, perdida: 0, perdidaMonto: 0, cuotasCount: 0, conocidas: 0, sinCosto: 0, factConocida: 0, orders: 0, count: 0 };
    for (const account of list) {
      ...
      T.cuotasCount += v.cuotasCount; T.conocidas += v.conocidas; T.sinCosto += v.sinCosto; T.factConocida += v.factConocida; T.orders += v.orders; T.count += v.count;
    }
...
function servePanel(mode, req, res) {
  ...
  // GESTIÓN: solo admin. Un usuario no-admin que intente entrar a /gestion vuelve al inicio.
  if (mode === 'gestion' && sess.role !== 'admin') { res.writeHead(302, { Location: '/' }); return res.end(); }
